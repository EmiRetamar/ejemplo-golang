...
ECS_SERVICE_NAME='SRV-Eco-Search-TEST'
ECS_TASK_DEFINITION_NAME='Eco-Search-TEST'
ECR_NAME='eco/back'
...
mv server.yml.TEST server.yml


DOCKERIMAGENAME="search-test"
# Docker operations
(
...
EOF
  ...
  aws ecs register-task-definition --region "${ECS_REGION}" --family "${ECS_TASK_DEFINITION_NAME}" --container-definitions "${NEW_CONTAINER_DEF}" --execution-role-arn "arn:aws:iam::798873886583:role/ecsTaskExecutionRole"
